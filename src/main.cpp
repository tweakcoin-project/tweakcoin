...
#if defined(NDEBUG)
# error "TweakCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2c76dce947b292aac9a6690490f0117024a6f99fde3e20e0de7b566b3d6564f6");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TweakCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TweakCoin Signed Message:\n";
...
{
    // TweakCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 25 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TweakCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TweakCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1489076409, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "We are happy to announce TweakCoin 09 march 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041834a30a26fab9675703d8a8a3513e9d533a47f68e6b31346a85e143edf96f3db46db0752bf9d8f67d0c6081956fcc33cc4130181bbd124f52a9f609a1ebc1a8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1489076409;
        ...
        assert(block.hashMerkleRoot == uint256("0x5a9079444ba31c587702cf0b4f9b858293afe86c1eed84f29c44ded2628a7f6c"));
...
unsigned char pchMessageStart[4] = { 0xc2, 0x2b, 0xff, 0x7d }; // TweakCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TweakCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TweakCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TweakCoinMiner(CWallet *pwallet)
{
    printf("TweakCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tweakcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TweakCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TweakCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TweakCoinMiner, pwallet));
}
